package projectthree.app.server.businessservice.encodecoboundedcontext.newdomain.aspect;
...
public class BusinessServicenewdomainAspect extends BusinessAspect {
    ...
    @Pointcut("execution(* projectthree.app.server.businessservice.encodecoboundedcontext.newdomain..*(..))")
    ...
    public Object aroundfindAll(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        Object object = null;
        try {
            Log.out.println("JKUUJ217100100", runtimeLogInfoHelper.getRequestHeaderBean(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName(), joinPoint.getSignature().getDeclaringTypeName() + ":" + joinPoint.getSignature().getName());
            ...
            Log.out.println("JKUUJ227100100", runtimeLogInfoHelper.getRequestHeaderBean(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName());
        } catch (Exception e) {
            methodCallDetails.setPostCallDetails(HealthConstants.METHOD_EXCEPTION, HealthConstants.DEFAULT_EXCEPTION_ID);
            AppAlarm appAlarm = Log.getExceptionAlarm("JKU", "UJ", e.getClass().getName());
