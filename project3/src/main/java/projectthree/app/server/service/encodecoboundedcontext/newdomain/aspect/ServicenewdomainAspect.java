package projectthree.app.server.service.encodecoboundedcontext.newdomain.aspect;
...
public class ServicenewdomainAspect extends ServiceAspect {
            ...
            if (checkToken(proceedingJoinPoint, request.getHeader("token"))) {
                if (request.getHeader("isPublicApi") != null) {
                    if (!validatePublicApi(request)) {
                        AppAlarm appAlarm = Log.getAlarm("JKUUJ154101500");
    ...
    public Object aroundAdvice2Save(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
        ...
        ResponseEntity<ResponseBean> responseEntity = new ResponseEntity<>(HttpStatus.OK);
        try {
            Log.out.println("JKUUJ114100100", runtimeLogInfoHelper.getRequestHeaderBean(), proceedingJoinPoint.getSignature().getDeclaringTypeName(), proceedingJoinPoint.getSignature().getName(), proceedingJoinPoint.getSignature().getDeclaringTypeName() + ":" + proceedingJoinPoint.getSignature().getName());
            ...
            Log.out.println("JKUUJ124100100", runtimeLogInfoHelper.getRequestHeaderBean(), proceedingJoinPoint.getSignature().getDeclaringTypeName(), proceedingJoinPoint.getSignature().getName(), proceedingJoinPoint.getSignature().getDeclaringTypeName(), methodCallDetails.getExecution_Time());
        } catch (Exception e) {
            AppAlarm appAlarm = Log.getExceptionAlarm("JKU", "UJ", EventAppLayers.WEB_SERVICE, EventAction.WRITE, e.getClass().getName());
            ResponseBean exceptionbean = new ResponseBean(appAlarm);
            exceptionbean.add("message", String.format(appAlarm.getMessage(), e.getMessage()));
            httpStatusCode = HttpStatus.valueOf(appAlarm.getAlarmStatus());
    ...
    public Object aroundAdvice2Update(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
        ...
        try {
            ...
            AppAlarm appAlarm = Log.getExceptionAlarm("JKU", "UJ", EventAppLayers.WEB_SERVICE, EventAction.UPDATE, e.getClass().getName());
            ResponseBean exceptionbean = new ResponseBean(appAlarm);
            exceptionbean.add("message", String.format(appAlarm.getMessage(), e.getMessage()));
            httpStatusCode = HttpStatus.valueOf(appAlarm.getAlarmStatus());
            methodCallDetails = setMethodPostCallDetails(methodCallDetails, HealthConstants.METHOD_EXCEPTION, httpStatusCode.name());
    ...
    public Object aroundAdvice2Delete(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
        ...
        try {
            ...
            AppAlarm appAlarm = Log.getExceptionAlarm("JKU", "UJ", EventAppLayers.WEB_SERVICE, EventAction.DELETE, e.getClass().getName());
            ResponseBean exceptionbean = new ResponseBean(appAlarm);
            exceptionbean.add("message", String.format(appAlarm.getMessage(), e.getMessage()));
            httpStatusCode = HttpStatus.valueOf(appAlarm.getAlarmStatus());
            methodCallDetails = setMethodPostCallDetails(methodCallDetails, HealthConstants.METHOD_EXCEPTION, httpStatusCode.name());
    ...
    public Object aroundAdvicefindOperation(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
        ...
        try {
            ...
            AppAlarm appAlarm = Log.getExceptionAlarm("JKU", "UJ", EventAppLayers.WEB_SERVICE, EventAction.READ, e.getClass().getName());
            ResponseBean exceptionbean = new ResponseBean(appAlarm);
            exceptionbean.add("message", String.format(appAlarm.getMessage(), e.getMessage()));
            httpStatusCode = HttpStatus.valueOf(appAlarm.getAlarmStatus());
            methodCallDetails = setMethodPostCallDetails(methodCallDetails, HealthConstants.METHOD_EXCEPTION, httpStatusCode.name());
    ...
    public Object aroundAdvice2AllOther(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
        ...
        try {
            ...
            AppAlarm appAlarm = Log.getExceptionAlarm("JKU", "UJ", EventAppLayers.WEB_SERVICE, EventAction.READ_WRITE_UPDATE, e.getClass().getName());
            ResponseBean exceptionbean = new ResponseBean(appAlarm);
            exceptionbean.add("message", String.format(appAlarm.getMessage(), e.getMessage()));
            httpStatusCode = HttpStatus.valueOf(appAlarm.getAlarmStatus());
            methodCallDetails = setMethodPostCallDetails(methodCallDetails, HealthConstants.METHOD_EXCEPTION, httpStatusCode.name());
    ...
    @Pointcut("execution(* projectthree.app.server.service.encodecoboundedcontext.newdomain..save*(..))")
    protected void saveOperation() {
    }

    @Pointcut("execution(* projectthree.app.server.service.encodecoboundedcontext.newdomain..update*(..))")
    protected void updateOperation() {
    }

    @Pointcut("execution(* projectthree.app.server.service.encodecoboundedcontext.newdomain..delete*(..))")
    protected void deleteOperation() {
    }

    @Pointcut("execution(* projectthree.app.server.service.encodecoboundedcontext.newdomain..find*(..))")
    protected void findOperation() {
    }

    @Pointcut("execution(* projectthree.app.server.service.encodecoboundedcontext.newdomain..get*(..)) && !within(com.spartan.server.session.bizService.SessionDataMgtService+)")
    protected void getOperation() {
    }

    @Pointcut("execution(* projectthree.app.server.service.encodecoboundedcontext.newdomain..*(..)) && !within(com.spartan.server.session.bizService.SessionDataMgtService+)")
    protected void allOperation() {
    }

    @Pointcut("execution(* projectthree.app.server.service.encodecoboundedcontext.newdomain..*(..)) && !within(com.spartan.server.session.bizService.SessionDataMgtService+) && ! findOperation() && ! saveOperation() && ! updateOperation() && ! deleteOperation()")
