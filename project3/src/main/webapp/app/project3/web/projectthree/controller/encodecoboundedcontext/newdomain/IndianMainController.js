Ext.define('Project3.project3.web.projectthree.controller.encodecoboundedcontext.newdomain.IndianMainController', {
     extend: 'Project3.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.IndianMainController',
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#IndianForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#IndianGrid');
               var tree = this.view.down('#IndianTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#IndianForm');
          var grid = this.view.down('#IndianGrid');
          var tree = this.view.down('#IndianTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#IndianGrid');
               ...
               var formPanel = this.getView().up().down('#IndianForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#IndianTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#IndianForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridConfig: function(data) {
          ...
          var gridStore = Ext.create('Ext.data.Store', {
               model: 'Project3.project3.shared.projectthree.model.encodecoboundedcontext.newdomain.IndianModel',
               autoLoad: true,
