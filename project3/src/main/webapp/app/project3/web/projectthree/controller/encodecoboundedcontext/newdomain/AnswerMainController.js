Ext.define('Project3.project3.web.projectthree.controller.encodecoboundedcontext.newdomain.AnswerMainController', {
     extend: 'Project3.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.AnswerMainController',
     init: function() {
          var form = this.view.down('#AnswerForm');
          form.reset();
     ...
     resetForm: function(but) {
          ...
          form.down('#saveFormButton').setText('Save');
          form.reset();
          var grid = this.view.down('#AnswerGrid');
          var tree = this.view.down('#AnswerTree');
          tree.setSelection();
          grid.setSelection();
     },
     hideDataBrowser: function() {
          var form = this.view.down('#Answer');
          var grid = this.view.down('#AnswerGrid');
          var tree = this.view.down('#AnswerTreeContainer');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#AnswerForm');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, opts) {
                    ...
                    var compRef = currentView.down('#AnswerTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          ...
          if (record.data.leaf) {
               var Answer = this.view.down('#AnswerForm');
               Answer.reset();
               var gridPanel = this.view.down('#AnswerGrid');
          ...
          var formPanel = this.getView().up().down('#AnswerForm');
     ...
     setGridConfig: function(data) {
          ...
          var gridStore = Ext.create('Ext.data.Store', {
               model: 'Project3.project3.shared.projectthree.model.encodecoboundedcontext.newdomain.AnswerModel',
               autoLoad: true,
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#AnswerTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Answer').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#AnswerGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
